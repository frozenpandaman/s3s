...
import s3s.iksm as iksm
import s3s.utils as utils
            ...
            if "latestBattleHistories" in query1_resp["data"]:
                for battle_group in query1_resp["data"]["latestBattleHistories"]["historyGroups"]["nodes"]:
                    for battle in battle_group["historyDetails"]["nodes"]:
                        # don't filter out private battles here - do that in post_result()
...
def prepare_battle_result(battle, ismonitoring, isblackout, overview_data=None):
    ...
    if mode == "BANKARA":
        try:
            payload["our_team_count"] = battle["myTeam"]["result"]["score"]
            payload["their_team_count"] = battle["otherTeams"][0]["result"]["score"]
        except BaseException:  # draw - 'result' is null
            pass
        ...
        if overview_data is None:  # no passed in file with -i
            overview_post = requests.post(
                ...
                cookies=dict(
                    _gtoken=GTOKEN))
            try:
                # make the request in real-time in attempt to get rank, etc.
                overview_data = [json.loads(overview_post.text)]
            except BaseException:
                overview_data = None
        ...
        if overview_data is not None:
            for screen in overview_data:
                if "bankaraBattleHistories" in screen["data"]:
                    ...
                    break
                # early exports used this, and no bankaraMatchChallenge below
                elif "latestBattleHistories" in screen["data"]:
            ...
            for parent in ranked_list:  # groups in overview (ranked) JSON/screen
                for idx, child in enumerate(parent["historyDetails"]["nodes"]):
                    ...
                    overview_battle_id_mutated = overview_battle_id.replace(
                        "BANKARA", "RECENT")  # same battle, different screens
                    ...
                    if overview_battle_id_mutated == battle_id_mutated:  # found the battle ID in the other file
                        ...
                        if "bankaraMatchChallenge" in parent and parent["bankaraMatchChallenge"] is not None:
                            ...
                            # rank-up battle
                            if parent["bankaraMatchChallenge"]["isPromo"]:
                            ...
                            if parent["bankaraMatchChallenge"]["udemaeAfter"] is not None:
                                if idx != 0:
                                    ...
                                    full_rank_after = re.split(
                                        '([0-9]+)', parent["bankaraMatchChallenge"]["udemaeAfter"].lower())
...
def prepare_job_result(job, ismonitoring, isblackout, overview_data=None):
    ...
    point_diff = 20 if payload["clear_waves"] == 3 else -30 + \
        (10 * job["resultWave"])  # +20 for win or -(30-10w) for loss
...
def post_result(data, ismonitoring, isblackout, istestrun, overview_data=None):
    ...
    if isinstance(data, list):  # -o export format
        try:
            # avoid {"data": {"vsHistoryDetail": None}} error
            data = [x for x in data if x["data"]["vsHistoryDetail"] is not None]
    ...
    for i in range(len(results)):
        ...
        if payload.get("lobby") == "private" and utils.custom_key_exists("ignore_private", CONFIG_DATA) or payload.get(
                "private") == "yes" and utils.custom_key_exists("ignore_private", CONFIG_DATA):  # SR version
            continue
...
def get_num_results(which):
    ...
    try:
        if which == "ink":  # TODO update '200' number when league released
            print(
                "Note: 50 recent battles of each type (up to 200 total) may be uploaded by instead manually exporting data with "
                '\033[91m' +
                "-o" +
                '\033[0m' +
                ".\n")
...
def monitor_battles(which, secs, isblackout, istestrun, skipprefetch):
    ...
    cached_battles, cached_jobs = fetch_json(
        which, separate=True, numbers_only=True, printout=True, skipprefetch=skipprefetch)
    if DEBUG:
    ...
    try:
        while True:
            ...
            which, cached_battles, cached_jobs, battle_wins, battle_losses, battle_draws, splatfest_wins, splatfest_losses, splatfest_draws, mirror_matches, job_successes, job_failures, foundany = check_for_new_results(
                *input_params)
        ...
        if which in ("ink", "both"):
            ...
            if splatfest_wins + splatfest_losses + splatfest_draws > 0:
                if splatfest_draws == 0:
                    print(
                        f"Splatfest: {splatfest_wins} win{'' if splatfest_wins == 1 else 's'} and "
                        f"{splatfest_losses} loss{'' if splatfest_losses == 1 else 'es'} against the other Splatfest team.")
                else:
                    print(
                        f"Splatfest: {splatfest_wins} win{'' if splatfest_wins == 1 else 's'}, "
...
def main():
    ...
    if outfile:
        ...
        parents, results, coop_results = fetch_json(
            "both", separate=True, exportall=True, specific=True, skipprefetch=True)
    ...
    if filenames:  # 2 files in list
        ...
        # one or multiple; monitoring mode = False
        post_result(to_upload, False, blackout, test_run, overview_data=overview_file)
        sys.exit(0)
